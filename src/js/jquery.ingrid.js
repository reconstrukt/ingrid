...
jQuery.fn.ingrid = function(o){
			...
			.extend({
				resizeStart : function(th, eventX){
					...
					jQuery(this).show().css({
						top: pos.top,
						left: eventX
					}).parents('.' + cfg.ingridBaseClass).find('table.' + cfg.gridClass).addClass('resizing');
					...
					jQuery('body').bind('mouseup', {col : th}, function(e){
						...
						jQuery('#' + z_sel).hide().parents('.' + cfg.ingridBaseClass).find('table.' + cfg.gridClass).removeClass('resizing');
						var th 		= e.data.col;
						var pos		= th.offset();
						var col_w	= e.clientX - pos.left;
						if (col_w > cfg.minColWidth) {
	...
	if (cfg.paging) {
		...
		if (cfg.totalRecords > 0) {
			...
			pb4	= jQuery('<a href="#">&raquo;</a>').addClass(cfg.pageEndClass).click(function(){
				var _p = p.getPage(); _p++;
				if (totp) {
	...
	console.log(this[0] + ' ...is a table')
	console.log(this.length + ' = total tables matched to selector')
	*/
	...
	return this.each(function(tblIter){
		...
		/*
		console.log(this + ' ...is a table [' + tblIter + '] , id="' + jQuery(this).attr('id') + '"')
		console.log(g[0] + ' ...is the grid div html');
		*/

		// append to doc
		var g_id = cfg.ingridIDPrefix + '_' + jQuery( this ).attr('id') + '_' + tblIter;
		g.addClass( cfg.ingridBaseClass ).attr( 'id', g_id );
